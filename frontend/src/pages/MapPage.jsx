...
import { Map, ListFilter, Compass, Navigation } from 'lucide-react';
...
export const MapPage = ({ userLocation, onRequestLocation }) => {
  ...
  useEffect(() => {
    loadData(initialSearch);
  }, [initialSearch, userLocation]);
  ...
  const loadData = async (query = '') => {
    ...
    try {
      ...
      if (query) {
        ...
        results = await fetchNearbyIdols(userLocation.lat, userLocation.lng);
      } else {
        results = await fetchAllIdols();
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
      ...
      <div className="bg-white p-4 rounded-2xl border border-orange-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end">
          ...
          <select
            value={ecoFilter}
            onChange={(e) => { setEcoFilter(e.target.value); loadData(); }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* Left Interactive Map (7 Cols on desktop) */}
        ...
        {/* Right Sidebar List (5 Cols on desktop) */}
        <div className={`lg:col-span-5 space-y-4 ${viewMode === 'map' ? 'hidden lg:block' : 'block'}`}>
          ...
          {loading ? (
            ...
            <div className="p-8 text-center bg-white rounded-2xl border border-orange-100 space-y-2">
              <p className="text-sm font-bold text-slate-700">No Ganesh Idols found matching search query.</p>
              <p className="text-xs text-slate-400">Try searching for Vijayawada, Mangalagiri, Guntur, Amaravati or clear search filters.</p>
            </div>
          ) : (
            <div className="space-y-4 max-h-[650px] overflow-y-auto pr-1">
              {idols.map((idol) => (
                <div
                  key={idol.id}
                  onMouseEnter={() => setSelectedIdolId(idol.id)}
                  onMouseLeave={() => setSelectedIdolId(null)}
                >
                  <IdolCard idol={idol} />
                </div>
              ))}
